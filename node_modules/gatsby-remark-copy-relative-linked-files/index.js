...
const isIgnore = (url, extentions) => {
  return Array.isArray(extentions) ? extentions.some(ext => url.endsWith(ext)) : false;
};
/**
...
module.exports = ({
  ...
  const visitor = link => {
    if (isAbsoluteURL(link.url) || isIgnore(link.url, checkIgnoreFileExtensions(pluginOptions.ignoreFileExtensions))) {
      return;
    }
